 ...
 * @version $Revision: 7 $
 */
public class LRUPruneHoeffdingTree extends AbstractClassifier implements MultiClassClassifier,
                                                                 CapabilitiesHandler {
    ...
    public IntOption LRUThresholdOption = new IntOption("LRUThreshold",
            'T',
            "The number of timestamps to be considered old (weighted by the depth of the tree).",
            600, 1, Integer.MAX_VALUE);
    ...
    public static class Node extends AbstractMOAObject {
        ...
        public double[] getClassVotes(Instance inst, LRUPruneHoeffdingTree ht) {
            return this.observedClassDistribution.getArrayCopy();
        }
        ...
        public void describeSubtree(LRUPruneHoeffdingTree ht, StringBuilder out,
                                    int indent) {
    ...
    public static abstract class LearningNode extends Node {
        ...
        public abstract void learnFromInstance(Instance inst, LRUPruneHoeffdingTree ht);
    }
    ...
    public static class InactiveLearningNode extends LearningNode {
        ...
        @Override
        public void learnFromInstance(Instance inst, LRUPruneHoeffdingTree ht) {
    ...
    public static class ActiveLearningNode extends LearningNode {
        ...
        public AttributeSplitSuggestion[] getBestSplitSuggestions(
                SplitCriterion criterion, LRUPruneHoeffdingTree ht) {
    ...
    public double getUsageFactor(int depth) {
        return 1 + Math.log(Math.pow(2,depth/2));
    }
    ...
    private int checkTreeUsage(Node node, int ts) {
        ...
        double wThresh = Math.ceil(this.pruneThreshold * this.getUsageFactor(depth));
        if (interval >= wThresh) {
    ...
    public int getNodeSubTreeDepth(Node subTreeRoot, Node wanted) {
        ...
        if (subTreeRoot != null) {
            ...
            // check the grandchildren of root
            for (Node c : l) {
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
      if (this.getClass() == LRUPruneHoeffdingTree.class)
